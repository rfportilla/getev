...
class AbsOcmapAttr(object):
    full_tree = 0
    __slots__ = ['_raw_data',]

    def __init__(self, data=None):
        self._raw_data = data
        self.__slots__ = self.__slots__ + [] if isinstance(self.schema, (list, tuple,)) else list(self.schema.keys())

    def __getitem__(self, key):
        if isinstance(self.raw_data, (list, tuple)):
            return self.schema[0](self.raw_data[key])

    def __repr__(self):
        if self.full_tree:
            return json.dumps(self.__dict__, indent=2)
        return str(pformat(self.raw_data, depth=2))

    def __eq__(self, value: object) -> bool:
        if hasattr(value, 'schema'):
            if isinstance(self, (list, tuple)) and isinstance(value, (list, tuple)):
                print('gothere - AbsOcmapAttr Compare (__eq__)')
                if len(self) != len(value):
                    print('gothere2')
                    return False
                return all(self[i] == value[i] for i in range(len(self)))
            return all(self[k] == value[k] for k in set(self.raw_data.keys()).union(set(value.raw_data.keys())))
        else:
            return False
    ...
    def __dir__(self):
        ...
        return super().__dir__() + list(self.schema.keys())
    ...
    def __getattr__(self, name: str) -> Any:
        if name in self.schema:
            if self.raw_data.get(name) is None:
                return None
            if isinstance(self.schema[name], (list, tuple)):
                klass = self.schema[name][0]
                r = [klass(x) for x in self.raw_data.get(name, []) if self.raw_data.get(name) is not None]
                return r
            return self.schema[name](self.raw_data[name])
    ...
    @classmethod
    def get_class(cls, schema:dict, name:str=None) -> type:
        # Create a class based on the given schema?  Or should it just be a named tuple?
        name = uuid.uuid4().hex if name is None else name
        c = type(name, (AbsOcmapAttr,), {'_schema': schema})
        return c


schema_user_info = { #Referenced in user comment
    'ID': int,
    'Username': str,
...
schema_user_comment_type = {
    'ID': int,
    'Title': str,
}

schema_checkin_status_type = {
    'ID': int,
    'Title': str,
    'IsAutomatedCheckin': bool,
    'IsPositive': bool,
}
schema_comment_checkin_response = {
    'status': str,
    'description': str,
}
schema_user_comment = {
    ...
    'ChargePointID': int,
    'CommentTypeID': int,
    'CommentType': AbsOcmapAttr.get_class(schema_user_comment_type, 'UserCommentType'),
    'UserName': str,
    'Comment': str,
    'RelatedURL': str,
    'DateCreated': datetime.datetime.fromisoformat,
    'User': AbsOcmapAttr.get_class(schema_user_info, 'UserInfo'),
    'CheckinStatusTypeID': int,
    'CheckinStatusType': AbsOcmapAttr.get_class(schema_checkin_status_type, 'CheckinStatusType'),
}
...
schema_media_item = {
    ...
    'CharePointID': str,
    'ItemURL': str,
    'ItemThumbnailURL': str,
    'Comment': str,
    'IsEnabled': bool,
    'IsVideo': bool,
    'IsFeaturedItem': bool,
    'IsExternalResource': bool,
    'User': AbsOcmapAttr.get_class(schema_user_info, 'UserInfo'),
    'DateCreated': datetime.datetime.fromisoformat
}

schema_media_item_response = {
    'chargePointID': int,
    'comment': str,
    'imageDataBase64': base64.b64decode
}
...
schema_address_info = {
    ...
    'AddressLine1': str,
    'AddressLine2': str,
    'Town': str, # Town or City
    'StateOrProvince': str, # State or Province
    'Postcode': str,
    'CountryID': int,
    'Country': [AbsOcmapAttr.get_class(schema_country, 'Country')],
    'Latitude': float,
    'Longitude': float,
    'ContactTelephone1': str,
    'ContactTelephone2': str,
    'ContactEmail': str,
    'AccessComments': str, #Guidance for users to use or find the equipment
    'RelatedURL': str,
    'Distance': float,
    'DistanceUnit': int, # 1: Miles, 2: KM; defaults to 1
    'Title': str, #General title for this location to aid user
}
...
schema_connection_type = {
    'FormalName': str, # Formal (standard) name for this connection type
    'IsDiscontinued': bool, # If true, this is an discontinued but used connection type
    'IsObsolete': bool, # If true, this is an obsolete connection type and is unlikely top be present in modern infrastructure
    'ID': int,
    'Title': str,
}

schema_status_type = {
    'IsOperational': bool, # Default is False
    'IsUserSelectable': bool, # Default is False
    'ID': int,
    'Title': str,
}
...
schema_level_type = {
    ...
    'Title': str,
    'Comments': str,
    'IsFastChargeCapable': bool,# If true, this level is considered 'fast' charging, relative to other levels.
}

schema_connection_info = {
    'ID': int,
    'ConnectionTypeID': int,
    'ConnectionType': AbsOcmapAttr.get_class(schema_connection_type, 'ConnectionType'),
    'Reference': str, # Optional operators reference for this connection/port; Defaults to None
    'StatusTypeID': int,
    'StatusType': AbsOcmapAttr.get_class(schema_status_type, 'StatusType'),
    'LevelID': int, # A general category for equipment power capability. Deprecated for general use. Currently computed automatically based on equipment power.
    'Level': AbsOcmapAttr.get_class(schema_level_type, 'LevelType'),
    'Amps': int,
    'Voltage': int,
    'PowerKW': int,
    'CurrentTypeID': int,
    'CurrentType': AbsOcmapAttr.get_class(schema_supply_type, 'SupplyType'),
    'Quantity': int, # Optional summary number of equipment items available with this specification
    'Comments': str,
}

schema_data_provider_status_type = {
...
schema_data_provider = {
    'WebsiteURL': str,
    'Comments': str,
    'DataProviderStatusType': AbsOcmapAttr.get_class(schema_data_provider_status_type),
    'IsRestrictedEdit': bool,
    'IsOpenDataLicensed': bool,
    'IsApprovedImport': bool,
    'License': str,
    'DateLastImported': datetime.datetime.fromisoformat,
    'ID': int,
    'Title': str,
}

schema_operator_info = {
    'WebsiteURL': str,
    'Comments': str,
    'PhonePrimaryContact': str,
    'PhoneSecondaryContact': str,
    'IsPrivateIndividual': bool,
    'AddressInfo': AbsOcmapAttr.get_class(schema_address_info, 'AddressInfo'),
    'BookingURL': str,
    'ContactEmail': str,
...
schema_poi = {
    'ID': int,
    'UUID': uuid.UUID,
    'UserComments': [AbsOcmapAttr.get_class(schema_user_comment, 'UserComment')],
    'MediaItems': [AbsOcmapAttr.get_class(schema_media_item, 'MediaItem')],
    'IsRecentlyVerified': bool,
    'DateLastVerified': datetime.datetime.fromisoformat,
    'ParentChargePointID': int,
    'DataProviderID': int,
    'DataProvidersReference': str,
    'OperatorID': int,
    'OperatorsReference': str,
    'UsageTypeID': int,
    'UsageCost': str,
    'AddressInfo': AbsOcmapAttr.get_class(schema_address_info, 'AddressInfo'),
    'Connections': [AbsOcmapAttr.get_class(schema_connection_info, 'ConnectionInfo')],
    'NumberOfPoints': int,
    'GeneralComments': str,
    'DatePlanned': datetime.datetime.fromisoformat,
    'DateLastConfirmed': datetime.datetime.fromisoformat,
    'StatusTypeID': int,
    'DateLastStatusUpdate': datetime.datetime.fromisoformat,
    'MetadataValues': [dict],
    'DataQualityLevel': int,
    'DateCreated': datetime.datetime.fromisoformat,
    'SubmissionStatusTypeID': int,
    'DataProvider': AbsOcmapAttr.get_class(schema_data_provider, 'DataProvider'),
    'OperatorInfo': AbsOcmapAttr.get_class(schema_operator_info, 'OperatorInfo'),
    'UsageType': AbsOcmapAttr.get_class(schema_usage_type, 'UsageType'),
    'StatusType': AbsOcmapAttr.get_class(schema_status_type, 'StatusType'),
    'SubmissionStatus': AbsOcmapAttr.get_class(schema_submission_status_type, 'SubmissionStatus'),
}

schema_pois = [
    AbsOcmapAttr.get_class(schema_poi, 'POI'),
]
...
schema_data = {
    'UserProfile': AbsOcmapAttr.get_class(schema_user_profile),
    'access_token': str,
}

schema_authentication_result = {
    'Data': AbsOcmapAttr.get_class(schema_data),
    'Metadata': AbsOcmapAttr.get_class(schema_metadata)    
}

schema_core_reference_data = {
    'ChargerTypes': [AbsOcmapAttr.get_class(schema_level_type, 'LevelType')],
    'ConnectionTypes': [AbsOcmapAttr.get_class(schema_connection_type, 'ConnectionType')],
    'CheckinStatusTypes': [AbsOcmapAttr.get_class(schema_checkin_status_type, 'CheckinStatusType')],
    'Countries': [AbsOcmapAttr.get_class(schema_country, 'Country')],
    'CurrentTypes': [AbsOcmapAttr.get_class(schema_supply_type, 'SupplyType')],
    'DataProviders': [AbsOcmapAttr.get_class(schema_data_provider, 'DataProvider')],
    'DataTypes': str,
    'MetadataGroups': str,
    'Operators': [AbsOcmapAttr.get_class(schema_operator_info, 'OperatorInfo')],
    'StatusTypes': [AbsOcmapAttr.get_class(schema_status_type, 'StatusType')],
    'SubmissionStatusTypes': [AbsOcmapAttr.get_class(schema_submission_status_type, 'SubmissionStatusType')],
    'UsageTypes': [AbsOcmapAttr.get_class(schema_usage_type, 'UsageType')],
    'UserCommentTypes': [AbsOcmapAttr.get_class(schema_user_comment_type, 'UserCommentType')],
}

schema_open_api_info = {
    'title': str,
    'version': str,
}

schema_open_api_result = {
    'openapi': str,
    'info': AbsOcmapAttr.get_class(schema_open_api_info, 'OpenApiInfo'),
}
...
'''
    ...
    d = json.loads(sample_data)
    
    
    # k = AbsOcmapAttr.get_class(schema_core_reference_data, 'CoreReferenceData')
    # o = k(d)
    # #o.full_tree = True
    # #print(o.ChargerTypes[0].ID)
    # print(o.Operators[0:3])
    
    d2 = json.loads(sample_poi)
    k2 = AbsOcmapAttr.get_class(schema_pois, 'POI')
    o = k2(d2)
    print(o.raw_data)
